...
### 2.2 Optional: Enable Rich Messages

Telegram Bot API 10.1 added Rich Messages with native headings, tables, math,
code blocks, expandable details, footnotes, and other formats. Enable them for
final agent replies and proactive messages:
...
This option is disabled by default so existing bots keep their current HTML
rendering. When enabled:

- Final `Reply` and `Send` output uses Telegram's native Rich Message Markdown.
- Messages longer than 32,768 characters are split while preserving code fences.
- Telegram API parsing or unsupported-method errors fall back to the existing
  HTML message path.
- Markdown images are sent as clickable links instead of remote media, preventing
  Telegram from fetching arbitrary URLs generated by an agent.
- Streaming progress edits and messages with inline buttons continue using the
  legacy message path.

Rich draft streaming (`sendRichMessageDraft`) is not enabled yet. Telegram limits
that method to private chats, so it will be added separately without changing
group/topic preview behavior.
